...
export async function analyzeJobMatchAction(formData: FormData) {
  ...
  if (!jobId) {
    if (!jobText.trim())
      throw new Error("Provide a Job ID or paste a job posting.");
    jobId = await createJobFromText({
      title: jobTitle,
      company: jobCompany,
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/analysis/match`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export async function deleteJobAction(jobId: string, currentCvId?: string) {
  ...
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL}/api/jobs/${encodeURIComponent(jobId)}`,
    {
      method: "DELETE",
