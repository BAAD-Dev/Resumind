...
const StateIcon = ({
  ...
  if (state === "uploading")
    return <div className="animate-spin rounded-full h-8 w-8 border-b-2" style={{borderColor: PRIMARY_BLUE}} />;
  if (state === "success")
    return (
      <svg
        className="h-8 w-8"
  ...
  if (state === "error")
    return (
      <svg className="h-8 w-8" style={{color: "#dc2626"}} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 font-sans px-2 sm:px-4">
      <div className="w-full max-w-xl bg-white rounded-xl shadow-xl p-2 sm:p-4">
       ...
       <div className="relative w-full max-w-xl bg-white rounded-xl shadow-xl p-2 sm:p-4">
        ...
        {/* Blue Section */}
        <div style={{background: PRIMARY_BLUE}} className="rounded-lg px-3 py-4 flex flex-col sm:flex-row items-center justify-between mb-3 gap-2">
          <div className="flex items-center gap-2">
            <InformationCircleIcon className="h-5 w-5 text-white" />
            <h2 className="font-semibold text-white text-base sm:text-lg">Upload Your Resume or CV</h2>
          </div>
          <button
        ...
        {/* Disclaimer Points */}
        <div className="mb-5 p-3 sm:p-4 border rounded-lg bg-blue-50" style={{borderColor: PRIMARY_BLUE}}>
        ...
        <div
          {...getRootProps()}
          className={`border-2 border-dashed rounded-lg p-5 sm:p-8 text-center transition-all duration-200 ${isDragActive ? "border-blue-700 bg-blue-50" : "border-gray-300 hover:border-blue-700 hover:bg-gray-50"}`}
        >
          <input {...getInputProps({ refKey: "ref" })} ref={inputRef} />
          <div className="flex flex-col items-center">
            <div className="p-2 sm:p-3 bg-gray-100 rounded-full mb-4" style={{border:`1px solid ${PRIMARY_BLUE}`}}>
              <ArrowUpTrayIcon className="h-8 w-8" style={{color: PRIMARY_BLUE}} />
            </div>
        ...
        {file && (
          <div className="mt-6">
            <div className="flex items-center justify-between p-2 sm:p-4 bg-gray-50 rounded-lg border" style={{borderColor:PRIMARY_BLUE}}>
            ...
            {status === "error" && (
              <div className="mt-3 p-2 sm:p-3 bg-red-50 border border-red-200 rounded-lg">
                <p className="text-red-800 text-xs sm:text-sm">{errorMessage}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-2 sm:px-4">
          ...
          <div className="relative z-10 w-full max-w-md rounded-xl bg-white p-4 sm:p-6 shadow-lg border border-blue-900">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-bold" style={{color: PRIMARY_BLUE}}>Legal & Privacy Disclaimer</h3>
              <button onClick={() => setShowDisclaimer(false)} className="p-2 hover:bg-gray-100 rounded cursor-pointer">
                <XMarkIcon className="h-5 w-5 text-gray-500" />
              </button>
            </div>
            <div className="text-xs sm:text-sm text-gray-800 space-y-3 mb-2">
              <div>
                <h4 className="font-semibold" style={{color:PRIMARY_BLUE}}>Privacy</h4>
                <p>Your Resume or CV is temporarily stored only for analysis purposes and deleted within 24 hours. We respect your privacy—your data is never shared or sold.</p>
              </div>
              <div>
                <h4 className="font-semibold" style={{color:PRIMARY_BLUE}}>AI Results</h4>
                <p>All results are generated by AI and may not be 100% accurate. Consider these only as suggestions, not definitive facts. Results may vary between analyses.</p>
              </div>
              <div>
                <h4 className="font-semibold" style={{color:PRIMARY_BLUE}}>Requirements</h4>
                <p>Only PDF files allowed, max size 5MB. Upload the correct format for best results.</p>
              </div>
              <div>
                <h4 className="font-semibold" style={{color:PRIMARY_BLUE}}>Legal Notice</h4>
                <p>This website provides analysis for information purposes only, and does not guarantee employment, admissions, or accuracy.</p>
              </div>
            </div>
            <div className="mt-3 flex justify-end">
              <button onClick={() => setShowDisclaimer(false)} className="bg-blue-600 hover:bg-blue-700 px-4 py-2 text-white rounded-lg font-medium cursor-pointer" style={{background:PRIMARY_BLUE}}>Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-2 sm:px-4">
          ...
          <div className="relative z-10 w-full max-w-lg bg-white rounded-xl shadow-lg border border-blue-900 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-base sm:text-lg font-bold" style={{color: PRIMARY_BLUE}}>Analysis Results</h2>
              <button
            ...
            {status === "success" && result && (
              <div className="space-y-4">
                <div className="p-3 sm:p-4 bg-green-50 border border-green-200 rounded-lg">
                  <h3 className="font-semibold text-green-900 mb-1 text-xs sm:text-base">Overall Score</h3>
                  <p className="text-green-800 text-xs sm:text-base">{result.overallScore}</p>
                </div>
                <div className="p-3 sm:p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h3 className="font-semibold" style={{color:PRIMARY_BLUE}}>Summary</h3>
                  <p className="text-blue-800 text-xs sm:text-base leading-relaxed">{result.summary}</p>
                </div>
              </div>
            )}
            <div className="mt-6 flex justify-end cursor-pointer">
              <button onClick={() => setShowResult(false)} className="bg-gray-100 hover:bg-gray-200 px-4 py-2 text-gray-800 rounded-lg font-medium">
                Close
              </button>
            </div>
          </div>
