...
function mapMidtransStatus(body: MidtransWebhookBody): OrderStatus {
  ...
  if (s === "capture")
    return body.fraud_status === "accept"
      ? OrderStatus.PAID
      : OrderStatus.PENDING;
...
// ====== This is the main webhook handler function with the email logic added ======
export async function handleMidtransWebhook(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (incomingStatus === OrderStatus.PAID && updatedOrder.userId) {
      // Find the user to get their name and email for the receipt
      const user = await tx.user.findUnique({
        where: { id: updatedOrder.userId },
      });

      if (user) {
        // Upgrade the user's role to PAID
        await tx.user.update({
          where: { id: user.id },
          data: { role: UserRole.PAID },
        });

        // 👇 THIS IS THE PERFECT SPOT
        // 📧 SEND THE RECEIPT EMAIL
        // We are inside the transaction, everything has been successfully updated.
        await EmailService.sendPaymentReceiptEmail(user.email, user.name, {
    ...
    if (
      ...
      const stillPaid = await tx.order.findFirst({
        where: {
          ...
          orderId: { not: updatedOrder.orderId },
        },
        select: { id: true },
      });
      if (!stillPaid) {
